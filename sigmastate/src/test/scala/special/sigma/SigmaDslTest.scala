...
class SigmaDslTest extends PropSpec
  ...
  property("Byte methods equivalence") {
    ...
    val arithOps = checkEq(func[Byte,Int](
       "{ (x: Byte) => x / 2 + (x / 2 - x / 4) * 2 - (max(x / 4, 0) - min(x / 4, 0)) }")
      ) { (x: Byte) => x / 2 + (x / 2 - x / 4) * 2 - (max(x / 4, 0) - min(x / 4, 0)) }

    val gt = checkEq(func[Byte,Boolean]( "{ (x: Byte) => x > 0.toByte }") ) { (x: Byte) => x > 0.toByte }
    val lt = checkEq(func[Byte,Boolean]( "{ (x: Byte) => x < 0.toByte }") ) { (x: Byte) => x < 0.toByte }
    val ge = checkEq(func[Byte,Boolean]( "{ (x: Byte) => x >= 0.toByte }") ) { (x: Byte) => x >= 0.toByte }
    val le = checkEq(func[Byte,Boolean]( "{ (x: Byte) => x <= 0.toByte }") ) { (x: Byte) => x <= 0.toByte }
    val eq = checkEq(func[Byte,Boolean]( "{ (x: Byte) => x == 0.toByte }") ) { (x: Byte) => x == 0.toByte }
    ...
    forAll { x: Byte =>
      Seq(toByte, toShort, toInt, toLong, toBigInt, arithOps, gt, lt, ge, le, eq).foreach(_(x))
    }
  ...
  property("Int methods equivalence") {
    ...
    val plus = checkEq(func[Int,Int]("{ (x: Int) => x / 2 + x / 2 }"))(x => (x / 2 + x / 2))
    ...
    forAll { x: Int =>
      Seq(toInt, toLong, toBigInt, plus).foreach(_(x))
  ...
  property("Short methods equivalence") {
    ...
    val plus = checkEq(func[Short,Short]("{ (x: Short) => x / 2 + x / 2 }"))(x => (x / 2 + x / 2).toShort)
    ...
    forAll { x: Short =>
      Seq(toShort, toInt, toLong, toBigInt, plus).foreach(_(x))
      //TODO soft-fork: toBytes, toBits, toAbs
    }
  ...
  property("Long methods equivalence") {
    ...
    val plus = checkEq(func[Long,Long]("{ (x: Long) => x / 2 + x / 2 }"))(x => x / 2 + x / 2)
    ...
    */
    ...
    forAll { x: Long =>
      Seq(toLong, toBigInt, plus).foreach(_(x))
      //TODO soft-fork: toBytes, toBits, toAbs
    }
    forAll { x: (Long, Long) =>
      //TODO soft-fork: compareTo(x)
