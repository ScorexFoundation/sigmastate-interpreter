...
class DistributedSigSpecification extends SigmaTestingCommons {
  ...
  property("distributed THRESHOLD mixed via AND") {
    ...
    val (rBob, aBob) = DLogInteractiveProver.firstMessage()
    val dlBKnown: Hint = RealCommitment(pubkeyDave, aBob)
    val secretCmtB = OwnCommitment(pubkeyDave, rBob, aBob)

    val (rEmma, aEmma) = DLogInteractiveProver.firstMessage()
    val dlEKnown: Hint = RealCommitment(pubkeyDave, aEmma)
    val secretCmtE = OwnCommitment(pubkeyDave, rEmma, aEmma)
