...
class DistributedSigSpecification extends SigmaTestingCommons {
  ...
  property("distributed THRESHOLD - 4 out of 8 - DH") {
    ...
    val bagH = HintsBag(Seq(dlAKnown, dlBKnown, dlGKnown))
    ...
    val validProofA = proverA.prove(prop, ctx, fakeMessage, bag.addHint(OwnCommitment(pubkeyAlice, rAlice, aAlice))).get
    verifier.verify(prop, ctx, validProofA, fakeMessage).get._1 shouldBe true

    val validProofB = proverB.prove(prop, ctx, fakeMessage, bag.addHint(OwnCommitment(pubkeyBob, rBob, aBob))).get
    verifier.verify(prop, ctx, validProofB, fakeMessage).get._1 shouldBe true

    val validProofG = proverG.prove(prop, ctx, fakeMessage, bag.addHint(OwnCommitment(pubkeyGerard, rGerard, aGerard))).get
    verifier.verify(prop, ctx, validProofG, fakeMessage).get._1 shouldBe true

   // val validProofH = proverH.prove(prop, ctx, fakeMessage, bag).get
