...
class IcoExample extends SigmaTestingCommons { suite =>
  ...
  property("simple ico example - fundraising stage only") {
    ...
    val projectBoxBefore = ErgoBox.create(10, fundingScript, 0, Seq(),
    ...
    val funderBoxes = (1 to funderBoxCount).map { _ =>
      ErgoBox.create(10, Values.TrueLeaf.asSigmaProp, 0, Seq(),
    ...
    val projectBoxAfter = ErgoBox.create(funderBoxCount * 10 - 1, fundingScript, 0, Seq(),
  ...
  property("simple ico example - issuance stage") {
    ...
    val projectBoxBeforeClosing = ErgoBox.create(10, issuanceScript, 0, Seq(),
    ...
    val projectBoxAfterClosing = ErgoBox.create(1, withdrawalScript, 0, Seq(tokenId -> projectBoxBeforeClosing.value),
      Map(R4 -> ByteArrayConstant(tokenId), R5 -> AvlTreeConstant(closedTreeData)))

    val ergoWithdrawalBox = ErgoBox.create(8, Values.TrueLeaf.asSigmaProp, 0, Seq(), Map())
    val feeBox = ErgoBox.create(1, feeProp, 0, Seq(), Map())
  ...
  property("simple ico example - withdrawal stage") {
    ...
    val withdrawBoxes = withdrawalAmounts.map { case (prop, tv) =>
      ErgoBox.create(1, DefaultSerializer.deserializeErgoTree(prop), 0, Seq(tokenId -> tv))
    }

    val totalTokenAmount = withdrawalAmounts.map(_._2).sum + 1

    val projectBoxBefore = ErgoBox.create(11, withdrawalScript, 0, Seq(tokenId -> totalTokenAmount),
      Map(R4 -> ByteArrayConstant(tokenId), R5 -> AvlTreeConstant(fundersTree)))
    val projectBoxAfter = ErgoBox.create(1, withdrawalScript, 0, Seq(tokenId -> 1),
