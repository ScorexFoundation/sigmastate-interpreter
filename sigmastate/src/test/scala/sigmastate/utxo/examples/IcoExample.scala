...
import sigmastate.Values.{AvlTreeConstant, IntArrayConstant, CollectionConstant, ByteArrayConstant, SigmaPropValue, ErgoTree, GroupElementConstant}
import sigmastate._
...
class IcoExample extends SigmaTestingCommons
  ...
  property("simple ico example - withdrawal stage") {
    ...
    val withdrawBoxes = withdrawalAmounts.map { case (prop, tv) =>
      testBox(1, DefaultSerializer.deserializeErgoTree(prop), 0, Seq(tokenId -> tv))
    }
