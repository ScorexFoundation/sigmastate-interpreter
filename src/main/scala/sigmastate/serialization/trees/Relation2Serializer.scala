...
// todo remove after transition to SigmaBuilder
 constraints: Seq[Constraints.TypeConstraint2]) extends ValueSerializer[R] {
...
//      assert(constraints.forall(c => c(firstArg.tpe, secondArg.tpe)))
      (constructor(firstArg, secondArg), r.consumed)
    }
  }

  override def serializeBody(rel: R): Array[Byte] = {
//    assert(constraints.forall(c => c(rel.left.tpe, rel.right.tpe)), s"constraints failed for $rel (left.tpe: ${rel.left.tpe}, right.tpe: ${rel.right.tpe})")
