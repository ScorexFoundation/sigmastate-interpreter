...
object ByteArrayConstantSerializer extends ValueSerializer[CollectionConstant] {

  override val opCode: OpCode = ByteArrayConstantCode
  val typeCode: TypeCode = SByteArray.typeCode

  override def parseBody(bytes: Array[Byte], pos: Position): (CollectionConstant, Consumed) = {
    val length = Shorts.fromByteArray(bytes.slice(pos, pos + 2))
    val consumed = 2 + length
    val array = bytes.slice(pos + 2 , pos + consumed)
    (CollectionConstant(array), consumed)
  }

  override def serializeBody(arr: CollectionConstant): Array[Byte] = {
