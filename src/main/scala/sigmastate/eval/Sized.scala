...
import special.collection.{CSizePrim, Size, CSizeOption, CollType, Coll, CSizeColl}
...
object Sized extends SizedLowPriority {
  ...
  implicit val BooleanIsSized: Sized[Boolean] = (x: Boolean) => new CSizePrim(1L, BooleanType)
  implicit val ByteIsSized: Sized[Byte] = (x: Byte) => new CSizePrim(1L, ByteType)
  implicit val ShortIsSized: Sized[Short] = (x: Short) => new CSizePrim(2L, ShortType)
  implicit val IntIsSized: Sized[Int] = (x: Int) => new CSizePrim(4L, IntType)
  implicit val LongIsSized: Sized[Long] = (x: Long) => new CSizePrim(8L, LongType)
  ...
  implicit val CollByteIsSized: Sized[Coll[Byte]] = (xs: Coll[Byte]) => {
    val byteSize = SByte.dataSize(0.asWrappedType)
    new CSizeColl(Colls.replicate(xs.length, new CSizePrim(byteSize, ByteType)))
  }
  ...
  implicit val boxIsSized: Sized[Box] = (b: Box) => {
    new CSizeBox(sizeOf(b.propositionBytes), sizeOf(b.bytes),  sizeOf(b.bytesWithoutRef), sizeOfRegisters(b))
  }
  ...
  implicit val contextIsSized: Sized[Context] = (ctx: Context) => {
    ...
    new CSizeContext(outputs, inputs, dataInputs, selfBox, rootHash, headers, preHeader)
  }
