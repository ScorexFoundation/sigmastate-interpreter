...
class MASTExampleSpecification extends SigmaTestingCommons {
  ...
  property("MAST") {
    ...
    val allSecrets = (0 until 5).map(i => Blake2b256(i.toString + " blinding factor"))
    val scriptBranches = allSecrets.map(s => OR(EQ(ByteArrayConstant(s), TaggedByteArray(secretId)), TrueLeaf))
    ...
    val (knownSecretTreeKey, knownSecret) = treeElements.head
    ...
    val prover = new ErgoProvingInterpreter()
      .withContextExtender(secretId, ByteArrayConstant(knownSecret))
