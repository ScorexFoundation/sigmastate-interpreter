...
class CoopExampleSpecification extends SigmaTestingCommons {
  ...
  property("commit to the threshold sig") {
    ...
    //todo: failback
    val spendingProp3 = compile(spendingEnv, "business").asBoolValue
    ...
    val thresholdEnv = Map(
      ...
      "spendingContract2Hash" -> ByteArrayConstant(Blake2b256(spendingProp2.bytes)),
      "spendingContract3Hash" -> ByteArrayConstant(Blake2b256(spendingProp3.bytes))
    )
    ...
    val outputToSpend = ErgoBox(totalValue, thresholdProp)
    val tx1Output1 = ErgoBox(toolValue, spendingProp1)
    val tx1Output2 = ErgoBox(constructionValue, spendingProp2)
    val tx1Output3 = ErgoBox(totalValue - toolValue - constructionValue, spendingProp3)
    val tx1 =  ErgoLikeTransaction(IndexedSeq(), IndexedSeq(tx1Output1, tx1Output2, tx1Output3))
    val ctx1 = ErgoLikeContext(
      ...
      self = outputToSpend)
    val proof1 = coopA.prove(thresholdProp, ctx1, fakeMessage).get.proof
    verifier.verify(thresholdProp, ctx1, proof1, fakeMessage) should be a 'success
