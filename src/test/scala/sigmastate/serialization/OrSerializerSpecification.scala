...
class OrSerializerSpecification extends TableSerializationSpecification {
  ...
  override def objects = Table(
    ("object", "bytes"),
    (OR(boolConst(true), boolConst(true)),  Array[Byte](OrCode, 0, 2, TrueCode, TrueCode)),
    (OR(boolConst(true), boolConst(false)), Array[Byte](OrCode, 0, 2, TrueCode, FalseCode)),
    (OR(boolConst(false),boolConst(true)),  Array[Byte](OrCode, 0, 2, FalseCode, TrueCode)),
    (OR(boolConst(false),boolConst(false)), Array[Byte](OrCode, 0, 2, FalseCode, FalseCode))
  )
