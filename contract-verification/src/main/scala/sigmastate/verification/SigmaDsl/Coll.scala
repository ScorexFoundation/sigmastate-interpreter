...
import sigmastate.verification.SigmaDsl.api.collection.{Coll => CollT}
...
object Coll {
  ...
  @extern @pure
  def empty[A]: CollT[A] = ??? //new CollOverList(List())

  @extern @pure
  def apply[A](a: A*): CollT[A] = ??? // new CollOverList(List(a))
