...
% two other privacy enhancing protocols. 
...
% (aka Composite Signatures)~\cite{composite-sigs}. 
...
% Due to this \algname offers better scalability. 
...
% \paragraph{Future enhancements:} Compared to CoinJoin, \algname requires an additional box, the Half-Mix box, as can be seen by comparing Figures~\ref{fig:coinjoin} and~\ref{fig:mixname}. One way to eliminate the extra box would be to have the mix step output two indistinguishable Half-Mix boxes that can be used again in the mix step (or spent elsewhere). 
...
In this approach, we will still use a Fee-Emission box (as in Section~\ref{donation}) to pay the fee in primary tokens. However, we will also use secondary tokens issued by the creator of the Fee-Emission box, which we call {\em mixing tokens} (identified by \texttt{tokenId}). The Fee-Emission box can only be used by burning some mixing tokens, i.e., by destroying a fixed amount of mixing tokens. 
...
val in0 = INPUTS(0).tokens(tokenId)
val in1 = INPUTS(1).tokens(tokenId)
val out0 = OUTPUTS(0).tokens(tokenId)
val out1 = OUTPUTS(1).tokens(tokenId)
val bobLogic = in1 > 0 && out0 == out1 && 
               in0 + in1 == out0 + out1 + 1 
...
\subsection{Obtaining Mixing Tokens}

While the earlier section focused on restricting how the mixing tokens are transferred after they are already in the system, this section will focus on defining how the tokens enter the system. 

The approximate-fairness strategy gives optimal results under two conditions. The first is to confine the mixing tokens within the system by restricting their transfer to only those boxes that participate in a remix, as we did above. 
...
This is done using a {\em Token-Emission box}, which we describe next. 
...
Multiple Token-Emission boxes should be created to handle several users. 
\ignore{
...
Assume for now that fee is paid in {\em mixing tokens}, which are secondary tokens\footnote{Every transaction may generate any quantity of at most one secondary token, whose ID is the box-ID of the first input. For other secondary tokens, the sum of quantities in outputs must be less than or equal to the sum of quantities in inputs.} issued by a 3rd party. We will later convert the protocol to use primary tokens. Creation of a mixed output consumes one mixing token. A mix transaction (which has two such outputs) consumes exactly two mixing tokens and, to maintain privacy, the balance tokens must be equally distributed between the two outputs. Below are some strategies to ensure fairness in fee payment. 
...
\begin{enumerate}
 ...
 This optimal fee strategy, however, has two drawbacks. Firstly, it has weakened privacy because it restricts the coins that can be mixed. Secondly, it impacts usability because there may not be boxes with the desired number of tokens. 
 
 The approximate fairness strategy, discussed next, has better privacy and usability at the cost of reduced fairness.
 ...
 However, 
she also requires Bob to have {\em initially} started with exactly, say, 1000 tokens in his first mix. Thus, if Bob is contributing, say, 1 token in the current mix, then Alice wants to ensure that he actually got there `the hard way', by starting out with 1000 tokens and losing them in sequential mixes. 
%, rather than just by starting out with 1. 
...
val sameTokenHalfBox = {(b:Box) => halfBox(b) && b.tokens(0) == tokens(0)}
...
\begin{verbatim}
OUTPUTS.forall({(o:Box) => o.R6[Coll[Byte]].get == blake2b256(propositionBytes)}
\end{verbatim}
This ensures that $R_6$ of each output 
contains its hash. The code of \texttt{fullMixScript} is modified:

\begin{verbatim}
val halfMixScriptHash = R6[Coll[Byte]].get
val halfBox = {(b:Box) => blake2b256(b.propositionBytes) == halfMixScriptHash}
val sameTokenHalfBox = {(b:Box) => halfBox(b) && b.tokens(0) == tokens(0)}
val noToken = {(token:(Coll[Byte], Long)) => token._1 != tokens(0)._1}
...
\section{\mixname Implementation}

Users will pay the fee in {\em mixing tokens}, which are secondary tokens issued by some entity, say Eve that need not be trusted. Eve's only purpose is to facilitate the protocol, and possibly make some profit in doing so. More specifically, her only role is to issue mixing tokens based on some rules. Since it is not easy to convince miners to accept fee in mixing tokens, we will use the idea of {\em Fee-Emission box} as described at~\cite{forum-post-fee-box}. The idea is to use mixing tokens as an accounting mechanism and exchange them for primary tokens to pay fee at the time of transaction creation. Eve may offer a lower buying rate (compared to the selling rate) for the tokens to make some profit, or she may buy them at a higher rate to encourage more people to use \mixname.

The complete implementation of \mixname requires the following types of boxes.
\begin{enumerate}
    \item \textbf{Token Seller Box:} This box contains mixing tokens issued by Eve. Tokens in this box can be purchased by participants for use in the protocol. The only restriction is that the tokens are purchased in batches of 1000 and stored in a {\em token buyer box}. 
    
    \item \textbf{Token Buyer Box:} This box is used to temporarily store tokens after purchasing from the token seller box. Tokens inside this box can only be spent if the spending transaction outputs a half mix box (in which case the entire tokens are transferred), or if the first input of transaction is a half mix box (in which case the rules encoded in the half mix box will apply). Additionally, there can only be one buyer box in a transaction. 
    
    \item \textbf{Half Mix Box:} This contains logic for spending the mixing tokens using approximate fairness. The amount of this box is hard coded to some fixed value, say 10 Ergs. 
    The box can only be spend if the output box contains exactly equal number of tokens, after removing two tokens from the inputs to cover fee. In particular:
    \begin{enumerate}
        \item It must be the first input box in the transaction. 
        \item The total number of mixing tokens of all inputs must be at least 2 more than what is in this box.
        \item It should output two full mix boxes as specified by protocol in indices 0 and 1. 
        \item Each full mix box must have (total - 2)/2 tokens rounded down. This ensures that each full mix box is spendable.
    \end{enumerate}
    
    \item \textbf{Full Mix Box:} This contains logic for preserving tokens if they are kept within the mixing pool. The value of primary tokens is hard-coded to the same as that of a half-mix box. Additionally, for any transaction spending a full mix box, if one of the following is not true than all mixing tokens are destroyed. 
    \begin{enumerate}
        \item The first input is a half mix box
        \item The first output is a half mix box and has one token less than the current box. 
    \end{enumerate}
    \item \textbf{Fee-Emission Box:} This box is used to exchange mixing tokens with fee in primary tokens. It has the logic that the output can contain one fee paying box with value a fixed multiple of the amount of mixing tokens destroyed in a transaction. The balance fee is stored in another Fee-Emission box. 
\end{enumerate}
}
