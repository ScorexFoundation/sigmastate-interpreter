...
Because of the condition \texttt{bob > 0} in the Half-Mix box, a mix requires Bob to supply at least one mixing token, and since these tokens can only be stored in Full and Half-Mix boxes, the second input of a mix transaction must be a Full-Mix box (as opposed to any type of box).
...
Figure~\ref{fig:mixingtokens} gives an example flow with the above contracts in place.
\begin{figure}[h]
	\centering
