...
\lstdefinestyle{mystyle}{
    ...
    basicstyle=\tiny \ttfamily, %\footnotesize, %or \small
...
\zerocoin is a privacy enhancing protocol that 
%depicted in Figure~\ref{fig:zerocoin}. The protocol 
uses a pool called the {\em unspent pool} (U-pool). A coin is added to this as a public commitment $c = \mathsf{Comm}(s, r)$ of secrets $s, r$ and later removed in a way
that does not reveal $c$ of the coin being removed. Instead, the removing transaction only reveals secret $s$, the ``serial number'', along with a zero-knowledge proof that one of the $c$'s in the U-pool is of the form $\mathsf{Comm}(s, r)$ for some $r$. Note that $c$ is stored in the U-pool permanently.
%The commitment scheme is such that revealing $s$ does not leak any information about $r$. Thus, the randomness $r$ remains secret. 
