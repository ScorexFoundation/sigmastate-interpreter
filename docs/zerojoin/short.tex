...
An interesting problem in CoinJoin-type protocols such as QuisQuis and \algname is the problem of fee. Since the protocol requires multiple rounds where a box's value is preserved across rounds. If the fee must be paid from an external source then privacy is broken. Thus, any fee must come from the boxes being mixed. However, then the value cannot be preserved.
Our solution to the fee problem is \mixname, an implementation of \algname on the Ergo blockchain. \mixname uses certain properties of Ergo (specifically its tokens) to develop a novel method to handle fee in such protocols. 
