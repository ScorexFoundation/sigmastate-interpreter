...
In this section, we describe somes two-party protocols that can be created with Ergo
...
\textbf{Offchain Pool:} Alice's Half-Mix box can be stored off-chain till the mixing step. Alice can either hand her unbroadcasted transaction directly to Bob or store it with a 3rd party service. Bob (or the 3rd party) will broadcast both Half and Full mix transactions after the mix step. It will be interesting to have a variant of the protocol where both Alice and Bob add Half-Mix coins to the H-Pool and any arbitrary 3rd party can combine them without interacting with Alice or Bob. 
...
For simplicity, assume that relevant transactions are mined by a 3rd party accepting fee in tokens\footnote{Tokens are additional assets stored in register R3 of a box as a collection of (token-ID, quantity) pairs. Every transaction may generate at most one token-ID of arbitrary quantity, where token-ID is the box-ID of the first input. For any other token-ID, the sum of quantities in outputs must be less or equal to the sum of quantities in inputs.} such that each box creation requires one token. A straightforward method of handling fee is as follows. Alice's Half-Mix box contains $i$ such tokens and requires each output box of the mix to contain exactly $i-1$ tokens. This will allow Alice to mix her coin $i$ times. While being perfectly fair, this method has usability issues because there may not be readily available boxes with the desired number of tokens.  
 
\begin{enumerate}
	\item \textbf{Approximate Fairness:} Bob will mix by picking from the H-pool a box with token quantity ``closer'' to that of his own non-H-pool box. He will generate two outputs with equal quantity of tokens after deducting fee. Alice's script could encode the maximum allowed difference in quantities. In the worst case, to mix $i$ times, Alice will have to start with a box of $2^i$ tokens.
		
	\item \textbf{Fee accumulation:} Fee can be accumulated rather than paid upfront. %For instance, fee for the mix transaction can be paid when spending the mixed outputs. 	
	Additionally, fee can keep accumulating as long as the mixed outputs are spent within the system (i.e., sent back to the H-pool or used in another mix operation). The accumulated fee is paid when the coin exits the system.
		The accumulated fee of the mix transaction is the sum of accumulated fee of the inputs plus mining fee. This accumulated fee is equally shared by the two mixed outputs. 
	
%	as long as the spending transaction either performs a pool or mix operation and the accumulated fee is paid at the time the coin leaves the mixing system. 
%	The accumulated fee for the mix transaction is the sum of accumulated fee of the inputs plus the mining fee for the mix transaction. To ensure privacy, this accumulated fee is equally shared by the two outputs. 
	\item \textbf{First spender pays:} Instead of sharing the (accumulated) fee equally in the mixed outputs, the first party to spend the mixed output should pay the entire (accumulated) fee, while the second party gets a free ride. This can be achieved using additional tokens as follows: 
	\begin{enumerate}
		\item Any box added to the H-pool must have exactly two quantities of a newly created token with token-ID $x$ (i.e., $x$ is the id of the first input box of the creating transaction). 
		\item A mix transaction must generate two more boxes each with one quantity of token $x$. 
		\item Spending any box with token $x$ must generate one box with two quantities of token $x$. The newly created box must also be protected by the same proposition.
		\item Spending any mixed output must generate one more box with two quantities of token $x$.
	\end{enumerate}
	Any transaction spending the mixed output is the second spender if there exists an input box with two quantities of token $x$, otherwise it is the first spender.
	
\end{enumerate}

%For simplicity, we can assume an intermediary providing the protocol as a service. This involves handling handling tokens and mining.
