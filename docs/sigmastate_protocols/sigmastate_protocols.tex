...
val script = compile(env, """ \Hi{\{}
  ...
  \Hi{val toKeep:Long = SELF.value - SELF.value * percent / 100}
  \Hi{val thisMin = if (toKeep > minSpend) toKeep else 0L}
  
  \Hi{val newStart:Int = if (notExpired) start else HEIGHT}
  \Hi{val newMin:Long = if (notExpired) min else thisMin}
  
  \Hi{val isValidOut = \{(out:Box) =>}
  ...
  \Hi{(alice && bob) || (}
    \Hi{min >= thisMin && // topup should keep min > thisMin}
    \Hi{(alice || bob) && (newMin == 0 || OUTPUTS.exists(isValidOut))}
  \Hi{) \}} """)
\end{alltt}
