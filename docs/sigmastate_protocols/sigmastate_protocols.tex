...
Consider the \mixname example, where we want to accumulate fee as long as the coin does not leave the system, In particular, we need to identify the mixed outputs that are either sent back to the H-pool or used as an input in another Full-Mix transaction.
...
Firstly, we require that the scripts not hardcode any keys. Thus, \texttt{gX} must be read from a register in both \texttt{fullMixScript} and the code in Alice's Half-Mix box, which we call \texttt{halfMixScript}. Additionally, \texttt{halfMixScript} must have code ensuring that the correct value of \texttt{gX} is copied in corresponding register of the mixed output boxes. 

Secondly, we require that one register of both the Half-Mix box and the two mixed output boxes will store the hash of another script called the \texttt{protocolCheck} script defined below. Additionally, \texttt{halfMixScript} must have code ensuring that the corresponding register of both mixed outputs contains the same hash. Now add the following code in \texttt{fullMixScript}:

\begin{alltt}
	\Hi{val protocolScriptHash = SELF.R7[Coll[Byte]].get}
	\Hi{val protocolScript = getVar[Coll[Byte]](1).get}
	\Hi{val protocolScriptMatches = blake2b256(protocolScript) == protocolScriptHash}
	\Hi{val protocolScriptAccepts = executeFromVar(1)}	
	\Hi{val spentWithinSystem = protocolScriptAccepts && protocolScriptMatches}
\end{alltt}

Finally, compute the hash: \texttt{val halfMixScriptHash = blake2b256(halfMixScript)}. Add both \texttt{halfMixScriptHash} and \texttt{fullMixScriptHash} to environment \texttt{env} and compute \texttt{protocolScript} as follows:

\begin{alltt}
val protocolScript = compile(env, """\Hi{\{}
  \Hi{val protocolScriptHash = SELF(0).R7[Coll[Byte]].get}
  \Hi{val isFullTx = OUTPUTS.size == 2 && INPUTS.size == 2 &&}
                 \Hi{blake2b256(OUTPUTS(0).propositionBytes) == fullMixScriptHash &&}
                 \Hi{blake2b256(OUTPUTS(1).propositionBytes) == fullMixScriptHash &&}
                 \Hi{blake2b256(INPUTS(0).propositionBytes) == halfMixScriptHash &&}
                 \Hi{OUTPUTS(0).R7[Coll[Byte]].get == protocolScriptHash &&}
                 \Hi{OUTPUTS(1).R7[Coll[Byte]].get == protocolScriptHash &&}
                 \Hi{INPUTS(0).R5[Coll[Byte]].get == protocolScriptHash}
	              
  \Hi{val isHalfTx = OUTPUTS.size == 1 && INPUTS.size == 1 &&}
                 \Hi{blake2b256(OUTPUTS(0).propositionBytes) == halfMixScriptHash &&}
                 \Hi{OUTPUTS(0).R5[Coll[Byte]].get == protocolScriptHash}
	
  \Hi{isHalfTx || isFullTx \}} """)
  
val protocolScriptHash = blake2b256(protocolScript)
\end{alltt}

The Half-Mix box is created with \texttt{protocolScriptHash} stored in its register R4. 
