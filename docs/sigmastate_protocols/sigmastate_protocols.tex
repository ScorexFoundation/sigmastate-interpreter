...
This requires that nothing be hardcoded in the scripts. Thus, \texttt{gX} must be read from a register in both \texttt{fullMixScript} and \texttt{halfMixScript}. Using the first available register, this is R6 in \texttt{fullMixScript} and R4 in \texttt{halfMixScript}. Additionally, \texttt{halfMixScript} must ensure that the correct \texttt{gX} is copied in the corresponding register (i.e., R6) of a mixed output. 

Secondly, one register of every box must store a 32 byte sequence. This is R7 in \texttt{fullMixScript} and R5 in \texttt{halfMixScript}. Additionally, \texttt{halfMixScript} must ensure that the appropriate register (i.e., R7) of each mixed output also contains the same value. We now extend \texttt{fullMixScript}:

\begin{alltt}
\Hi{val hash = SELF.R7[Coll[Byte]].get // hash of halfMixScript} 
\Hi{val isHalf = blake2b256(OUTPUTS(0).propositionBytes) == hash &&}
             \Hi{INPUTS.size == 1 && OUTPUTS(0).tokens(0) == SELF.tokens(0)}
\Hi{val isFull = blake2b256(INPUTS(0).propositionBytes) == hash && INPUTS.size == 2} 
\Hi{val isInternal = full || half} 
\end{alltt}

Finally, we compute \texttt{val hash = blake2b256(halfMixScript)} and create a Half-Mix box with \texttt{hash} stored in its register R4. 
...
\Hi{val isHalfMixBox = \{(b:Box) =>}
  \Hi{b.R5[Coll[Byte]].get == halfMixScriptHash &&}
  \Hi{blake2b256(OUTPUTS(0).propositionBytes) == halfMixScriptHash \}}
  
\Hi{isHalfMixBox(INPUTS(0)) || (isHalfMoxBox(OUTPUTS(0)) &&}
