...
%	A transaction creating Alice's box for the H-pool must generate exactly 4 quantities of a new token stored in her box. Let $x$ be the token-ID of this token. Alice requires that a transaction spending her box must generate four boxes each with 1 quantity of token $x$ as follows:
	
	A mix transaction must generate exactly 4 quantities of a new token (with some id $x$) distributed equally among 4 output boxes as follows:
	\begin{enumerate}
		\item The two standard mix outputs $O_0, O_1$ having the additional spending condition that the sum of quantities of token $x$ in the inputs is $\geq 3$.
...
%		\item A mixed box can only be spent if the sum of quantities of token $x$ in the inputs is $\geq 3$.
	...
	Thus, a transaction spending $O_0$ or $O_1$ is the second spender if and only if there exists an input box with two quantities of token $x$. This condition can be checked in the script when computing the (accumulated) fee.%, otherwise it is the first spender.
