...
\textbf{Offchain Pool:} Alice's Half-Mix box can be stored off-chain till the mixing step. Alice can either hand her unbroadcasted transaction directly to Bob or store it with a 3rd party service. Bob (or the 3rd party) will broadcast both Half and Full mix transactions after the mix step. It will be interesting to have an offchain variant of the protocol where both Alice and Bob add their Half-Mix coins to the H-Pool and a 3rd party can combine them without interacting with Alice or Bob. 
...
Assume that fee will be paid in tokens\footnote{Tokens are additional assets stored in register R3 of a box as a collection of (token-ID, quantity) pairs. Every transaction may generate at most one token-ID of arbitrary quantity, where token-ID is the box-ID of the first input. For any other token-ID, the sum of quantities in outputs must be less or equal to the sum of quantities in inputs.} and mined by some 3rd party.
...
%	The accumulated fee for the mix transaction is the sum of accumulated fee of the inputs plus the mining fee for the mix transaction. To ensure privacy, this accumulated fee is equally shared by the two outputs. 
	...
	A transaction creating Alice's box for the H-pool must generate exactly 4 tokens and those should be present in Alice's box. Let $x$ be the token-ID of this token. A mix transaction must generate four boxes each with 1 quantity of token $x$ as follows:
	\begin{enumerate}
		\item Two standard mixed boxes $O_0, O_1$ with the additional spending condition that the sum of quantities of token $x$ in the inputs is $\geq 3$.
		\item Two new boxes $O_2, O_3$ with the following identical spending conditions:
		\begin{enumerate}
			\item The spending transaction must have one output box containing 2 quantities of token $x$ and protected by the same script as this box. 
			\item Exactly one output box in the spending transaction can contain token $x$.
		\end{enumerate}
%		\item A mixed box can only be spent if the sum of quantities of token $x$ in the inputs is $\geq 3$.
	\end{enumerate}
	
	The first spender will spend both $O_2$ and $O_3$ to generate a new box with 2 quantities of token $x$ and spendable only by the second spender. 
	Thus, a transaction spending $O_0$ or $O_1$ is the second spender if and only if there exists an input box with two quantities of token $x$. %, otherwise it is the first spender.
