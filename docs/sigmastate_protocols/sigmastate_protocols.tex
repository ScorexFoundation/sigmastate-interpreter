...
\section{Introduction to $\Sigma$-protocols}
\label{intro:sigma}
...
We give some examples of \langname to illustrate its usage. The full code corresponding to the snippets below is available at the \langname code repository.
...
\begin{verbatim}
  ...
  alice || { validBobInput && blake2b256(out.propositionBytes) == scriptHash &&
...
Let $a, b\in \mathbb{Z}_3$ be the choices Alice and Bob, with the understanding that 0, 1 and 2 represent rock, paper and scissors respectively. If $a = b$ then the game is a draw, otherwise Alice wins if $a-b \in \{1, -2\}$ else Bob wins. The game is similar to XOR, except that Bob must now generate two outputs. In the draw case each player gets one output, otherwise the winner gets both. 
...
The above code ensures that \texttt{R7} of the first output contains Alice's public key (for the draw scenario). Bob should ensure that \texttt{R7} of the second output contains his public key. 


\subsection{Mixing Protocol}
...
To encode this protocol in \langname, first define two variables representing $g$ and $g^x$. Lets call them \texttt{g} and \texttt{g\_x} respectively. Then pass them to the compiler via an environment variable, say \texttt{env}:

\begin{verbatim}
val fullMixScript = compile(env, """{
  val e = SELF.R4[GroupElement].get
  val f = SELF.R5[GroupElement].get
  proveDlog(f) || proveDHTuple(g, e, g_x, f) }""")
\end{verbatim}
...
blake2b256(OUTPUTS(0).propositionBytes) == scriptHash &&
blake2b256(OUTPUTS(1).propositionBytes) == scriptHash &&
OUTPUTS(1).R4[GroupElement].get == d && OUTPUTS(1).R5[GroupElement].get == c && 
OUTPUTS.size == 2 && {proveDHTuple(g, g_x, c, d) || proveDHTuple(g, g_x, d, c)}
\end{verbatim}
...
any funds sent to a reversible address can only be spent using a {\em reversible transaction}. That is, transactions spending funds from such an address must create outputs that allow funds to be reversed for a certain time. 
...
Compute \texttt{scriptHash = Blake2b256(withdrawScript)}. Then hardcode \texttt{alice} and \texttt{scriptHash} inside \texttt{depositEnv} to create a compiled script called \texttt{depositScript}:
