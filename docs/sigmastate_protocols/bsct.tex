...
%As the name suggests, UTXOs are the outputs of a transaction that are not yet spent, and can encode arbitrary conditions as long as they are expressible in the underlying script.
...
uses the {\em account-based} model, where funds are stored in long-lived accounts and can be transferred in fractional amounts~\cite{wood2014ethereum}. End users operate an {\em externally owned account}, whose address is (the hash of) a public key. Smart contracts, on the other hand, exist on an address derived from the creating transaction in a deterministic way. %A smart contract has methods that can be invoked by sending a message from another address. 
...
%Due to lack of space, we don't give the fundamentals of \langname, for which 
...
%The spender of a box must provide a `proof' of satisfying its spending condition. 
We refer the reader to \langname resources for details~\cite{langrepo,tutorial,advtutorial}.
%the tutorials available online at
%~\cite{langrepo,whitepaper}. %However, we give a brief overview below.
