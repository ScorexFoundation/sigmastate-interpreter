...
\begin{tabularx}{\textwidth}{| l | X |}
  ...
  \bf{Serialized as} & \hyperref[sec:serialization:operation:SizeOf]{\lst{SizeOf(opCode=177)}} \\
  \hline
...
\lst{default} & \lst{: IV} & \text{// value to return when \lst{index} is out of range} \\
  ...
  \bf{Serialized as} & \hyperref[sec:serialization:operation:ByIndex]{\lst{ByIndex(opCode=178)}} \\
  \hline
...
\begin{tabularx}{\textwidth}{| l | X |}
 ...
 \lst{f} to each element of this collection and collecting the results.
  ...
  \bf{Serialized as} & \hyperref[sec:serialization:operation:MapCollection]{\lst{MapCollection(opCode=173)}} \\
  \hline
...
Returns \lst{true} if the given predicate \lst{p} is satisfied by at least one element of this collection, otherwise \lst{false}
  ...
  \bf{Serialized as} & \hyperref[sec:serialization:operation:Exists]{\lst{Exists(opCode=174)}} \\
  \hline
...
\lst{op} & \lst{: (OV,IV) => OV} & \text{// the binary operator} \\
  ...
  \bf{Serialized as} & \hyperref[sec:serialization:operation:Fold]{\lst{Fold(opCode=176)}} \\
  \hline
...
holds for all elements of this collection, otherwise \lst{false}.
  ...
  \bf{Serialized as} & \hyperref[sec:serialization:operation:ForAll]{\lst{ForAll(opCode=175)}} \\
  \hline
...
\lst{until} & \lst{: Int} & \text{// the lowest index to EXCLUDE from this collection} \\
  ...
  \bf{Serialized as} & \hyperref[sec:serialization:operation:Slice]{\lst{Slice(opCode=180)}} \\
  \hline
...
\begin{tabularx}{\textwidth}{| l | X |}
 ...
 predicate \lst{p}. The order of the elements is preserved.
  ...
  \bf{Serialized as} & \hyperref[sec:serialization:operation:Filter]{\lst{Filter(opCode=181)}} \\
  \hline
...
\begin{tabularx}{\textwidth}{| l | X |}
  ...
  \bf{Serialized as} & \hyperref[sec:serialization:operation:Append]{\lst{Append(opCode=179)}} \\
  \hline
