...
$Term_{\langname}$ &  & $Term_{Core}$  \\	
	\hline
$\Low{ \TyLam{x}{T}{e} 		}$ & \To & $\TyLam{x}{T}{ \Low{e} }$ \\	
...
$\Low{ \IfThenElse{e_{cond}}{e_1}{e_2} }$ & \To & 
		$\Apply{(\IfThenElse{ \Low{e_{cond}} }{ \Lam{(\_:Unit)}{\Low{e_1}} }{ \Lam{(\_:Unit)}{\Low{e_2}} })}{}$ \\ 
...
evaluation of its branches whereas right-hand-side \lst{if} belongs to
\corelang and have strict evaluation of the branches. The laziness is
achieved by using lambda expressions of \lst{Unit} $\to$ \lst{Boolean} type.
