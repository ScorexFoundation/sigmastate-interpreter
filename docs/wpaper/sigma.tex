...
Before we describe the language, let us fix some terminology. A \emph{box} contains some amount (\emph{value}, measured in Ergo tokens) and is protected by a \emph{script}\dnote{our box also contains: 1. additionalTokens: Seq[(TokenId, Long)] 2. id of transaction, created this box and index in this transaction, 3. Additional registers}. A \emph{transaction} spends the value of boxes that are its \emph{inputs} (which are outputs of some earlier transaction) and produces boxes that are its \emph{outputs}. In a given transaction, the sum of the values of the inputs must equal the sum of the values of the outputs\dnote{Rule for additionalTokens is similar: for every token except of token with id == first input id, sum of token in inputs should equal sum of this token in inputs. See \url{https://github.com/ergoplatform/ergo/blob/513e7309e5e2c1243fddaaf8bcac97a92cb70382/src/main/scala/org/ergoplatform/modifiers/mempool/ErgoTransaction.scala\#L117} for implmentation}.
\lnote{check -- what about rewards and mining etc.}
\dnote{Fees are implemented as boxes with TrueLeaf as a proposition, last transaction in a block consumes all this just created boxes and in favor of miner. All coins are created in genesis block in a box, that is protected by special case of emission script, that allows every miner to take only part of this coins. See CoinEmissionSpecification and https://github.com/ergoplatform/ergo/blob/513e7309e5e2c1243fddaaf8bcac97a92cb70382/src/main/scala/org/ergoplatform/local/ErgoMiner.scala\#L200}.
All the unspent transaction outputs (\emph{UTXO}) at a given time represent the value stored in the blockchain. A script for a box must evaluate to ``true'' when this box is used as an input to a transaction. This evaluation is helped by a \emph{proof} (for $\Sigma$-statements) and a \emph{context}, which are part of the transaction. The proof is produced by someone who knows the relevant secrets, such as secret keys; the context contains information about the transaction, such as details of its inputs and outputs, and the current state of the blockchain, such as the current number of blocks (height) and the root of the Merkle tree\dnote{Actually our AVL+ tree is used here} that contains the UTXO set.
\lnote{someone please check this?}
\dnote{This should change in future, e.g. fixed number of last headers is going to be added here (and header contains height and root hash).}
...
\lnote{when is it enough to just write pk and when do you need proveDlog(pk)? I see that ComplexSigSpecification uses the former, while ErgoLikeInterpreterSpecification (property ``Prove keys from registers'') uses the latter.}

\mnote{ In ComplexSigSpecification \texttt{pubkeyA} has Scala type ProveDlog
and \langname type Boolean. In ErgoLikeInterpreterSpecification pubkey1 has
type GroupElement and proveDlog is a function returning Boolean. However,
both cases have semantic problems. In the first case ProveDlog is not
Boolean, because Boolean have two values True and False and ProveDlog is
neither of them. In second case proveDlog computing Boolean (i.e. True
$\vert$ False) from group element which doesn't make sense at all. Why it
works? Because current implementation uses special Trick, it allow ProveDlog
to be in position where Boolean value is expected and reduction procedure
(reduceToCripto) knows what to do with this. Thus proveDlog function
constructs ProveDlog(ge) from given ge and returns it as if it is Boolean
(which is not, but remeber The Trick! ). ProveDlog is special not only
because it CAN be used in place of Boolean, but also because it CANNOT be
stored in context parameter and in register of the Box. The reason for this
again lies in types. Suppose we want to put some value of ProveDlog type in
context variable with id=1. Then we can access this value from \langname in
expression like \texttt{getVar[Boolean](1) \&\& c2 $\vert \vert$ c3}. This is
ok, because of The Trick. However, the following is illegal
\texttt{getVar[Boolean](1).propBytes}, because Boolean cannot have such
property, and the type checker of \langname rejects this expression. Because
it see that getVar returns Boolean and Booleans cannot have properties.
}

In order for the compiler to know what value the variable \texttt{pk} is referring to, the compiler needs to be supplied with an \emph{environment}, which, in this case, is a single-element map, mapping the string \texttt{"pk"} to the object holding the public key. Thus, the constant public key is hardwired into the script at compile time. When the script is later evaluated (i.e., when the box is used as a transaction input), a $\Sigma$-proof of knowledge of the corresponding secret key must be supplied by the prover. 
\lnote{it is unclear to me how to create environments outside of Scala code, so I am not exactly sure what to write here, because typed objects live inside Scala. The description in LangSpec.md is confusing me: I understand that you want variables of type Sigma to be parameters to the script, but in most situations they need to specified (hardwired as constants) when the script is created, because the script has specific public keys in mind. So how do you specify which public keys you have in mind? The runtime shouldn't be able to change that specification.}

\mnote{All global names in \langname are resolved at compile time.
Compile time is when script text is transformed to a serialized byte array,
which is later put into the blockchain. Environment is a compile-time data
structure to resolve names, it is used by the compiler and should be created
by the application before the compiler invocation. I planned to use
parameters of the \texttt{guard} construction shown in LangSpec.md examples.
...
\end{verbatim}
uses the predefined variable HEIGHT, which is the sequential block number (since the beginning of the blockchain  
\lnote{check?}\dnote{first block have index 0, may be it's better to write something like "number of ancestors of this block since the beggining"})
...
\lnote{check?} \dnote{true}

To access information inside a box \texttt{b}, scripts can use \texttt{b.value} for the amount, \texttt{b.propositionBytes} for the protecting script, and \texttt{b.id} for the identifier of the box, which is the BLAKE2b-256 hash of the contents of the box. Boxes include additional information in \emph{registers};  each box is unique, because ones of its registersincludes the transaction id in which it was created as an output, and its own index in the \texttt{OUTPUTS} array, accessible through \texttt{b.R3} (more on registers later in this section).
\lnote{Is there a better name than  \texttt{b.R3}, similar to \texttt{b.value}?}
\dnote{ErgoBox contains 10 registers R0-R9, value is kept in R0, script is kept in R1, tokens are kept in R2, link to reference is kept in R3, other registers are optional and does not have predefined type}
...
A context can also contain typed variables that can be retrieved by numerical id using the operator \texttt{getVar}. 
\lnote{I don't quite understand where variables that we get via \texttt{getVar} live. Are they part of the transaction? How do you get them into the context? Does the prover supply them? Does the miner, as part of the transaction?}\dnote{Prover can add this variables to his proof. Box proof (`ProverResult`) consist of `proof`(signature) and `ContextExtension` - this additional variables}
This can be useful, for example, for requiring a spending transaction to produce hash preimages (the BLAKE2b-256 and SHA-256 hash functions can invoked in \langname, using keywords \texttt{blake2b256} and \texttt{sha256}). For example,
\begin{verbatim}
       pkA && blake2b256(getVar[Array[Byte]](1)) ==  hashOutput
\end{verbatim}
says that spending can be done only using the signature of Alice, and only if the preimage of \texttt{hashOutput} is written in the context. Specifically, the script requires that the context should contain a variable (number \texttt{1} in the context) which an array of bytes that hashes to the value of \texttt{hashOutput} (the value of \texttt{hashOutput}, like \texttt{pkA}, is defined in the environment and is hardwired into the script at compile time).  Note that although the script requires both the secret key corresponding to \texttt{pkA} and the hash preimage corresponding to \texttt{hashOutput}, there is the stark difference between how these two values are used: the secret key is not revealed in the proof (by the zero-knowledge property of $\Sigma$-proofs), while the hash preimage is explicitly written into the context and can be seen by anyone.

\lnote{how high can numerical ids go -- is it sequential or are arbitrary numbers okay? Are they the numerical ids scoped per script or the same for all scripts in a given transaction? Why numerical and not string ids?}
\dnote{This numerical ids are of type Byte, so they in a range from -128 to 127. They are not sequential, arbitrary numbers are okay. This ids scoped per script and are defined by prover separatly for every input}
...
\lnote{why do you need \texttt{heightIncreased}? How can be it be that height is correct but not increased?}
\dnote{Miner may try to take reward twice inside the same block. E.g. first transaction of the block takes this output O, and creates new output O1. Second transation takes this just created output O1 and creates output O2. heightIncreased is the only rule that forbides miner to do this}
...
\dnote{I think example of rule 110 implementation from https://arxiv.org/abs/1806.10116 will be useful here, since it proves Turing completeness of our language and ErgoScript code was not included into original paper}
