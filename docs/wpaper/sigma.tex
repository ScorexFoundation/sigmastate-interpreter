...
The idea behind the language is that a subset of zero-knowledge protocols known as $\Sigma$-protocols (sigma protocols) could be combined via $\land$ and $\lor$ connectives forming complex statements like ``prove me a knowledge of discrete logarithm of (publicly known) $x_1$ or knowledge of Pedersen commitment $x_2$ preimage''. We make an observation that sigma protocol statements and their conjectures are homomorphic to boolean values, and we can add arbitrary boolean predicates to statements provable via a $\Sigma$-protocol, if both prover and verifier are able to evaluate the predicates in exactly the same way. This is the case if predicates are evaluated deterministically in the same way by both the prover and the verifier, and inputs for the predicates are the same on both sides. We use predicates over state of blockchain system during script validation~(which happens when a transaction tries to spend an output protected by the script). To avoid inefficient processing and impossibility for a light client to validate a transaction, this state is very lean but nevertheless it is richer than in Bitcoin. Like in Bitcoin, we use current height of the blockchain, but also a spending transaction with outputs it creates and inputs it tries to spend. Unlike Bitcoin, we allow outputs to contain more fields than amount and protecting script, in a form of additional registers an output can have. In addition to $\land$ and $\lor$ connectives, we have different operations over statically typed arguments. We reject out during compilation time expressions with type errors, like $2 + 2 > true$, thanks to a type system used.
...
We use $dlog(x)$ to denote a statement ``prove a knowledge of such $w$ that $x = g^w$''. Proving is to be done by a prover in zero knowledge~(with no presenting the secret $w$ to a verifier). Statement $dlog(x_1) \land dlog(x_2)$ means ``prove a knowledge of both $w_1$ and $w_2$, such as $x_1 = g^{w_1}, x_2 = g^{w_2}$'', similarly, $dlog(x_1) \lor dlog(x_2)$ is about a proof of knowledge of either $w_1$ or $w_2$.
...
\subsection{Sigma to Boolean Homomorphism}

Cryptocraphic proofs verifiable via sigma-protocols are represented by values of type $Sigma$.
There are two binary operations over sigma values:
\begin{itemize}
\item $\lor: (Sigma, Sigma) -> Sigma$
\item $\land: (Sigma, Sigma) -> Sigma$
\end{itemize}

This operations allow to construct complex cryptographic statements out of basic cryptographic
primitives (such as $dlog$, which is also of type $Sigma$).
Any expression of $Sigma$ type can be verified using sigma protocol.

If $H: Sigma -> Boolean$ is verification procedure, then for our purposes it should satisfy the following properties. For any two $sigma1, sigma2: Sigma$:
\begin{itemize}
    \item $H(sigma1 \lor sigma2) = H(sigma1) || H(sigma2)$
    \item $H(sigma1 \land sigma2) = H(sigma1) && H(sigma2)$
\end{itemize}

%H: Sigma -> Boolean
%изоморфизма тут нет, но есть гомоморфизм H: Sigma -> Boolean.
%Который в данном случае выражается равенством H(sigma1 \/ sigma2) = H(sigma1) || H(sigma2)
%Аналогично H(sigma1 /\ sigma2) = H(sigma1) && H(sigma2)
%Это свойство будет сохранено
%s: Sigma, x: Boolean, s \/ x := H(s) || x
%
%и кстати H(sigma1 \/ sigma2) = H(sigma1) || H(sigma2) должно быть симметричным
%т.е. действовать в обе стороны
%
%из того что я знаю один из секретов для sigma1, sigma2 следует что я знаю секрет для sigma1 или я знаю секрет для  sigma2?
%
%в протоколах есть нюанс, что неизвестно, какой именно из серкретов ты знаешь
%сохранен ли он здесь?
%
%нет, H(sigma1) || H(sigma2) такой вариант содержит дополнительный бит информации
%известно какой знаем левый или правый
%в этом и есть разница типов
%поэтому если не хочется этот бит раскрывать то не надо делать трансформацию
%H(sigma1 \/ sigma2) --> H(sigma1) || H(sigma2)
%
%и это не относится к языку, это к тому как на нем писать
%можно написать не то что хотел, понять это и исправить
%можно сделать анализатор количества раскрываемой информации
%который по программе скажет где и что раскрывается и можно ли оптимизировать
%это интересное потенциальное развитие
%для первой версии можно и без этого всего
%
