...
Conceptually, $\Sigma$-protocols \cite{Cra96} are generalizations~\cite{CL06} of signature schemes.
In fact, Schnorr signature scheme~\cite{Sch91} (whose more recent version is popularly known as EdDSA \cite{BDLSY12,rfc8032}) is the canonical example of a non-interactive $\Sigma$-protocol: it proves that the recipient knows the discrete logarithm of the public key (the proof is attached to a specific message, such as a particular transaction, and thus becomes a signature on the message; all $\Sigma$-protocols described here can be attached to specific messages). $\Sigma$-protocols exist for proving a variety of properties and, importantly for ErgoScript, elementary $\Sigma$-protocols can be combined into more sophisticated ones using the techniques of \cite{CDS94}. For an introduction to $\Sigma$-protocols, we refer the reader to \cite{Dam10} and \cite[Chapter 6]{HL10}.
...
\section{ErgoScript Language Description}
Before we describe the language, let us fix some terminology. A \emph{transaction} spends \emph{inputs} (which are outputs of some earlier transaction) and produces \emph{outputs}. Inputs and outputs are protected by scripts.  In order for a transaction to spend an input, the transaction must contain a \emph{proof} that satisfies its protecting script. The script has access to \emph{context}, which contains information about the transaction, such as its inputs and outputs and the scripts protecting them, and the current state of the blockchain, such as the current number of blocks (height) and the root of the Merkle tree that contains all the unspent outputs. \lnote{someone please check this?}

\paragraph{Simple Scripts}
The simplest script allows the owner of a public key to spend an output in a future transaction by issuing a signature with the corresponding secret key. If the variable \texttt{pk} holds the public key, then this script is specified simply as a string
\begin{verbatim}
        pk
\end{verbatim}

In order for the compiler to know what \texttt{pk} is referring to, the compiler needs to supplied with an \emph{environment}, which, in this case, is a single-element map, mapping the string \texttt{"pk"} to the object holding the public key. Thus, the constant public key is hardwired into the script at compile time.\lnote{it is unclear to me how to create environments outside of Scala code, so I am not exactly sure what to write here, because typed objects live inside Scala.}

A slightly more complex script may allow either one of two people to spend an output. If Alice owns public key \texttt{pkA}  and Bob owns public key \texttt{pkB} (with corresponding secret key \texttt{skA} and \texttt{skB}), then the script that would allow either one of them to spend the output is
\begin{verbatim}
        pkA || pkB
\end{verbatim}
Again, \texttt{pkA} and \texttt{pkB} need to mapped to public key objects by an \emph{environment} map \lnote{fix}. Note Alice and Bob individually can construct the proof necessary to spend this ouput using just one of the two corresponding secret keys, and, by the zero-knowledge property of $\Sigma$-protocols, the proof will not reveal whether \texttt{skA} or \texttt{skB} was used. This construct is known as a \emph{ring} signature.
...
\begin{verbatim}
        pkA && pkB
\end{verbatim}
requires Alice and Bob to use secret keys corresponding to \texttt{pkA} and \texttt{pkB} in order to spend the output. Note that Alice and Bob will have to communicate to produce the proof: separate signatures by Alice and Bob will not be sufficient (but Alice and Bob will not have to reveal their secret keys to each other). Similarly to \texttt{||}, multiple \texttt{\&\&} can be used in sequence, and the operator \texttt{allOf} can be used for arrays.

Using these operators, it is possible to produce more sophisticated scripts. For example, here is a script stating that the output can be spent either by Carol or by a collaboration between Alice and Bob:
...
\paragraph{Mixing $\Sigma$-statements with other statements}
ErgoScript allows combining statements that require proofs with other boolean  statements. These statements can refer to the context, which contains information about the transaction and has predefined variables. For example, the script
\begin{verbatim}
   pkA || pkB || (pkC && HEIGHT > 500)
\end{verbatim}
uses the predefined variable HEIGHT, which is the sequential block number (since the beginning of the blockchain) of the future block in which the script will be executed in order to spend the output it is protecting. 


transaction will resides. Thus, when the

allows Alice or Bob to spend the output before HEIGHT reaches 501, and Alice, Bob, or Carol to spend the output after that. If the height has not reached 501 and the ouput is spent, then the proof of course reveals that the proof used \texttt{skA}or \texttt{skB}, but not which one of the two. If HEIGHT is greater than 500, then the proof does not reveal which of the three secret keys was used. Thus, depending on the value of HEIGHT, the script becomes either equivalent to \texttt{pkA || pkB} or equivalent to \texttt{pkA || pkB || pkC} \lnote{someone define HEIGHT}

ErgoScript allows references to the information contained in the transaction. \lnote{simple example here}

ErgoScript also allows for a context \lnote{can someone give a rough description of what context is and where it lives and then I can take it from there?}. A context contains typed variables that can be retrieved by id using the operator \texttt{getVar}. This can be useful, for example, for requiring a spending transaction to produce hash preimages (the BLAKE2b-256 hash function is built into ErgoScript) \lnote{any other hashe functions built in?} For example,
\begin{verbatim}
       pkA && blake2b256(getVar[Array[Byte]](1)) ==  hashOutput
\end{verbatim}
says that spending can be done only using the signature of Alice, and only if the context contains a variable (number \texttt{1} in the context) which an array of bytes that hashes to the value of \texttt{hashOutput} (this value, like \texttt{pkA}, is defined in the environment and is hardwired into the script at compile time).

\section{More interesting examples here}
