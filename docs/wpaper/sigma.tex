...
\end{itemize}
\lnote{This is not the notation used in the code, but in the code, $g$ is overloaded because it's the base point in item 1 and an arbitrary point in item 2; $h$ is also overloaded. Perhaps we should change the code to better notation and then make this text match it.}
...
\lnote{Should we add, for every code example, a link to the code where it
appears? That would help the reader.} \mnote{This would be really useful but
currently the examples are spread all over the code base. We first need to
collect them in a single place (package or file).}
...
\lnote{If we want to show examples of the underlying syntax tree to which the language gets compiled, we can do that here, in parallel with examples in the language}

The simplest script allows the owner of a public key to create an output box
which can be spent in a future transaction by issuing a signature with the
corresponding secret key. If the variable \texttt{pk} holds the public key,
then this script is specified simply as a string\mnote{We can start
description with introducting function PK("...") as motivated in \footnote{
https://github.com/ScorexFoundation/sigmastate-interpreter/issues/230}}

\begin{verbatim}
val pk: SigmaProp = PK("<base64 encoding of public key>")
pk.isValid
\end{verbatim}

\mnote{The environment is really misleading here.
It is not part of the language, but rather specific way of integration of the language in Scala runtime.
The language itself should be self-contained.
In particular it should be possible to introduce constant values of all types using the language itself.
The val definitions above is exactly equivalent to how the environment is used by the compiler
to embed constants in the ErgoTree.}
\mnote{We say, our language is statically typed, but we don't show the types of the values.
In particular the reader will guess the type of pk. We need to refer to~\ref{sec:sigmaprop}}
...
We emphasize that evaluation of \texttt{\&\&} and \texttt{||} over proofs is
not the same as the usual left-to-right lazy evaluation of logical
expressions over booleans, because expressions involving $\Sigma$-statements
are not treated the same way as usual boolean expressions: they are always
evaluated in zero-knowledge and always inside special ZK scope of the script
(see~\ref{sec:sigmaprop}~and~\ref{sec:zk-scope}).
In fact,
$\texttt{pkA}$ is not of type \texttt{Boolean} \lnote{do we use
\texttt{Boolean} or \texttt{Bool}?}. \mnote{We should stick to Boolean, same
as in Scala} It is of type \texttt{SigmaProp}, which is special type for
sigma proofs (see~\ref{sec:sigmaprop}). The values of type \texttt{SigmaProp}
are used differently by the prover (who constructs the proof) and the
verifier (who checks it). \mnote{The following is no longer true: "moreover,
variables
of this type cannot be stored in boxes (Section \ref{sec:context}) or context
extensions (Section \ref{sec:extension})." } 

\lnote{the type system is undergoing changes, so this needs fixing}
\mnote{I think the connection between Boolean, SigmaProp, isValid, and
zero-knowledge scoping is now described in \ref{sec:sigmaprop} and
\ref{sec:zk-scope}. Please check.}
...
\mnote{Maybe make sense to introduce variable x explicitly?}
...
The fourth register, \texttt{b.R3}, contains a 34-byte array. This array specifies the 32-byte transaction id \lnote{how is transaction id computed?} where this box appears as an output followed by a 2-byte sequence number of this box in the \texttt{OUTPUTS} array of that transaction. This ensures that each box has unique \texttt{R3} and therefore a unique \texttt{id} as long as there are no hash collisions (because the \texttt{id} of the box is computed by hashing its content, including \texttt{R3}\mnote{It looks like self reference, in order to compute id we need R3, and R3 contains id.}).
...
Cryptocraphic propositions verifiable via $\Sigma$-protocols are represented
by values of an algebraic data type \texttt{SigmaProp}, which stands for
\emph{sigma proposition}. The values of type \texttt{SigmaProp}
are used differently by the prover (who constructs the proof) and the
verifier (who checks it). The type can be described using the following
declaration (using Scala notation).

\begin{verbatim}
trait SigmaProp
case class TrivialSigma(b: Boolean) extends SigmaProp
case class Dlog(g: GroupElement) extends SigmaProp
case class Dht (gv: GroupElement, hv: GroupElement,
                uv: GroupElement, vv: GroupElement) extends SigmaProp
...
\end{verbatim}
In the definition above, each constructor represents one of the cryptographic
primitives, and the set of such constructors can be extended (although this
extensibility is out of paper's scope).\mnote{How to describe the role of TrivialSigma?}

It is possible to construct more complex statements out of basic primitive propositions.
There are two binary operations over \texttt{SigmaProp} values:
\begin{itemize}
\item \texttt{\&\&: (SigmaProp, SigmaProp) -> SigmaProp}
\item \texttt{||: (SigmaProp, SigmaProp) -> SigmaProp}
\end{itemize}

Any expression of \texttt{SigmaProp} type can be efficiently verified using sigma protocol and thus mapped to a truth values.

Verification procedure \texttt{ZK: SigmaProp -> Boolean} performs such
mapping, and satisfies the following naturality conditions with respect to
\texttt{SigmaProp} and Boolean connectives.

For any two statements \texttt{s1, s2: SigmaProp}:
\begin{itemize}
    \item \texttt{ZK(s1 \&\& s2) = ZK(s1) \&\& ZK(s2)}
    \item \texttt{ZK(s1 || s2) = ZK(s1) || ZK(s2)}
\end{itemize}

Note, the operations \texttt{\&\&} and \texttt{||} are overloaded and work
for both \texttt{SigmaProp} (on the left) and \texttt{Boolean} (on the right).

Because SigmaProp connectives and logical connectives satisfy naturality conditions we can reason about complex sigma statements
...
Thus, we can verify the truth of \texttt{s1 || s2} (i.e. compute
\texttt{ZK(s1 || s2)}) statement but we cannot know whether \texttt{s1} or
\texttt{s2} or both is true. At the same time evaluation of \texttt{ZK(s1) ||
ZK(s2)} discloses results of both \texttt{ZK(s1)} and \texttt{ZK(s2)} before
execution of \texttt{||}.

The type system of \langname allows to explicitly delimit usages of zero knowledge evaluation
of sigma statements and classical evaluation of boolean statements (see~\ref{sec:zk-scope}).

This allows programmer to explicitly control security (zero-knowledge) guarantees where it is necessary.

\subsection{ZK scopes}
\label{sec:zk-scope}
