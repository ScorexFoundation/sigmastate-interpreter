...
The simplest script allows the owner of a public key to spend the transaction output. If the variable \texttt{pk} refers to the public key, then the script is specified simply as a string
\begin{verbatim}
        pk
\end{verbatim}

In order for the compiler to know what \texttt{pk} is referring to, the compiler needs to supplied with an \emph{environment}, which, in this case, is a single-element map, mapping the string \texttt{"pk"} to the object holding the public key. \lnote{it is unclear to me how to create environments outside of Scala code, so I am not exactly sure what to write here, because typed objects live inside Scala.}

A slightly more complex script may allow either one of two people to spend the transaction output. If Alice owns public key \texttt{pkA}  and Bob owns public key \texttt{pkB} (with corresponding secret key \texttt{skA} and \texttt{skB}), then the script that would allow either one of them to spend the output is
\begin{verbatim}
        pkA || pkB
\end{verbatim}
Again, \texttt{pkA} and \texttt{pkB} need to mapped to public key objects by an \emph{environment} map \lnote{fix}. Note Alice and Bob individually can construct the proof necessary to spend this transaction using just one of the two corresponding secret keys, and, by the zero-knowledge property of $\Sigma$-protocols, the proof will not reveal whether \texttt{skA} or \texttt{skB} was used. This construct is known as a \emph{ring} signature.

Naturally, this approach can be extended to more than two keys. If Carol's public and secret key are \texttt{pkC} and \texttt{skC}, then a three-key ring signature can be similarly constructed as
\begin{verbatim}
        pkA || pkB || pkC
\end{verbatim}
\lnote{I tried this and it didn't seem to work --- it is really not okay without parentheses?}
...
\begin{verbatim}
        pkA && pkB
\end{verbatim}
requires Alice and Bob to use secret keys corresponding to \texttt{pkA} and \texttt{pkB} in order to spend the transaction output. Note that Alice and Bob will have to communicate to produce the proof required to spend the transaction: separate signatures by Alice and Bob will not be sufficient (but Alice and Bob will not have to reveal their secret keys to each other). Similarly to \texttt{||}, multiple \texttt{\&\&} can be used in sequence, and the operator \texttt{allOf} can be used for arrays.

Using these operators, it is possible to produce a transaction that can be spent, for example, either by Carol or by a collaboration between Alice and Bob:

\begin{verbatim}
        (pkA && pkB) || pkC
\end{verbatim}

A valid proof for the spending transaction will not reveal which of two possibilities was used.

\paragraph{Mixing $\Sigma$-statements with other statements}
ErgoScript allows combining statements that require proofs with other boolean for statements. For example, the script
\begin{verbatim}
   (pkA || pkB) || (pkC && HEIGHT > 500)
\end{verbatim}
\lnote{are parentheses around \texttt{pkA || pkB} needed?}
allows Alice or Bob to spend the output before HEIGHT reaches 501, and Alice, Bob, or Carol to spend the output after that. If the height has not reached 501 and the transaction is spent, then the proof of course reveals that the proof used \texttt{skA}or \texttt{skB}, but not which one of the two. If HEIGHT is greater than 500, then the proof does not reveal which of the three secret keys was used. Thus, depending on the value of HEIGHT, the script becomes either equivalent to \texttt{pkA || pkB} or equivalent to \texttt{pkA || pkB || pkC} \lnote{someone define HEIGHT}

ErgoScript allows references to the information contained in the output transaction. \lnote{simple example here}
