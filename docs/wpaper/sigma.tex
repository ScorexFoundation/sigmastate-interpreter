...
In order for the compiler to know what the variable \texttt{pk} is referring to, the compiler needs to supplied with an \emph{environment}, which, in this case, is a single-element map, mapping the string \texttt{"pk"} to the object holding the public key. Thus, the constant public key is hardwired into the script at compile time. When the script is later evaluated (i.e., when the box is used as a transaction input), a $\Sigma$-proof of knowledge of the corresponding secret key must be supplied by the prover. \lnote{it is unclear to me how to create environments outside of Scala code, so I am not exactly sure what to write here, because typed objects live inside Scala.}
...
\lnote{should probably do a diffie-hellman example here}
...
\end{verbatim}
uses the predefined variable HEIGHT, which is the sequential block number (since the beginning of the blockchain  \lnote{check?}) of the block in which the script is evaluated.
...
In addition to the predefined variable \texttt{HEIGHT}, the context contains predefined arrays \texttt{INPUTS} and \texttt{OUTPUTS}, which refer to the inputs and outputs of the transaction. Elements of these arrays are of type \texttt{Box}. The script also has access to its own box via the context variable \texttt{SELF} of type \texttt{Box}. Note that \texttt{SELF} is also an element of the \texttt{INPUTS} array, because the script is executed when the its box is being spent. \lnote{check?} To access information inside a box \texttt{b}, scripts can use \texttt{b.value} for the amount and \texttt{b.propositionBytes} for the protecting script.\lnote{refere somewhere below for other stuff inside a box}

Access to this information allows us to create a script for the following crowdfunding situation: a project backer wishes to give money to a project, but only if the project raises enough money from other sources by a deadline. Let \texttt{backerPubKey} and \texttt{projectPubKey} be the public keys of the project backer and of the project, and let \texttt{minToRaise} be the amount that the project must raise by the deadline. To give money to the project, the backer will create an output box protected by the following script. The script contains two conditions: condition \texttt{c1}  for the case \texttt{HEIGHT >= deadline} (enabling the backer to spend the money) and condition  \texttt{c2} for the case \texttt{HEIGHT  < deadline} (enabling the project to spend the money if the amount is at least \texttt{minToRaise}).  In order to ensure enough money has been raised (as part of \texttt{c2}), the script will examine whether the transaction contains an output box with at least as much money as \texttt{minToRaise} that is being sent to the \texttt{projectPubKey}.

\begin{verbatim}
        let c1 = HEIGHT >= deadline && backerPubKey
        let c2 = allOf (Array (
           HEIGHT < deadline,
           projectPubKey,
           OUTPUTS.exists(fun (out: Box) = {
                        out.value >= minToRaise && 
                        out.propositionBytes == projectPubKey.propBytes
                })
        ))
        c1 || c2
\end{verbatim}

Note the use of operator \texttt{exists} on the array \texttt{OUTPUTS}: it returns true if the array contains an element that satisfies the boolean function which is passed in as an argument to \texttt{exists}. Functions are defined by the \texttt{fun} keyword; in this case, the function examines a box to see that its value is high enough and that its script is the simple script that allows to project to spend the value.

\lnote{this should probably move lower} The context also contains the root hash of the all the unspent output boxes in the previous block \lnote{check: previous or current block? It would seem that the current is not available, so it should be previous.}, available via the predefined variable \texttt{LastBlockUtxoRootHash}.



\paragraph{Context Extension}
A context can also containe typed variables that can be retrieved by id using the operator \texttt{getVar}. This can be useful, for example, for requiring a spending transaction to produce hash preimages (the BLAKE2b-256 hash function is built into \langname). For example,
\begin{verbatim}
       pkA && blake2b256(getVar[Array[Byte]](1)) ==  hashOutput
\end{verbatim}
says that spending can be done only using the signature of Alice, and only if the context contains a variable (number \texttt{1} in the context) which an array of bytes that hashes to the value of \texttt{hashOutput} (this value, like \texttt{pkA}, is defined in the environment and is hardwired into the script at compile time). SHA-256 hash function is similarly built into \langname and can be accessed via sha256 keyword.

\lnote{I don't quite understand where variable that we get via \texttt{getVar} live. Are they part of the transaction? I understand that \texttt{HEIGHT}, \texttt{INPUTS}, \texttt{OUTPUTS} are all part of the transaction, and therefore are available to the script. But I don't understand where numbered variables live. How do you get them into the context?}
