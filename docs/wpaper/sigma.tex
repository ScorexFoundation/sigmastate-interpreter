...
Our implementation of these protocols uses \lnote{references to bouncycastle and scapi and anything else}
...
\lnote{should probably do a diffie-hellman example, and maybe with getting some of the DH values from a context. E.g., $g_1$ is just $g$, $g_2$ is $pk_A$, $h_1$ is a hash of a message in a context, and $h_2$ is from the context}
...
\dnote{Rule for additionalTokens is similar: for every token except of token with id == first input id, sum of token in inputs should equal sum of this token in inputs. See \url{https://github.com/ergoplatform/ergo/blob/513e7309e5e2c1243fddaaf8bcac97a92cb70382/src/main/scala/org/ergoplatform/modifiers/mempool/ErgoTransaction.scala\#L117} for implmentation}.
...
\lnote{Not including demurrage example because it required more explanation about checking from miners and consensus about what registers are for what}
 ...
 \dnote{All coins are created in genesis block in a box, that is protected by special case of emission script, that allows every miner to take only part of this coins. See CoinEmissionSpecification and https://github.com/ergoplatform/ergo/blob/513e7309e5e2c1243fddaaf8bcac97a92cb70382/src/main/scala/org/ergoplatform/local/ErgoMiner.scala\#L200}.
