...
%\title{Scalable Smart Contracts using Immutable Objects and Decidable Programs}
...
%Contract chaining is a unique feature of Ergo and is not possible in other UTXO based blockchains. 

%and a technique called {\em contract chaining} that can emulate persistent storage in Ergo. Consequently, we are able to use 

%ashow how a UTXO based blockchain can emulate persistent storage using {\em contract trees}. We also describe this can be used in a new general-purpose blockchain platform called Ergo that that uses this to provide Ethereum-like functionality. 

% In this work, we show how persistent storage can be emulated on UTXO-based blockchains using a method called {\em contract chaining}.  called Ergo that can {\em contract chaining}, a method for emulating persistent storage on UTXO-based blockchains and 
 

%can be used t{\em Ergo}, a general-purpose platform based on Bitcoin's UTXOs design that focusses on functionality without reducing scalability. 
...
%Instead of UTXOs, Ethereum~\cite{wood2014ethereum} uses the {\em account-based} model, where funds are stored in long-lived accounts and can be transferred in fractional amounts~\cite{wood2014ethereum}. End users operate an {\em externally owned account}, whose address is (the hash of) a public key. Smart contracts, on the other hand, exist on an address derived from the creating transaction in a deterministic way. Ethereum contracts can maintain a long-lived global state, making them quite powerful. 
...
%All the examples presented here use contract chaining.
...
% Give overview of smart contract applications. Should we focus on only financial contracts/apps?
% Good example of non-financial app, such as Invention protection, Land records, etc

% Other properties
% - Efficient verification
% - Minimal state stored
% - Deterministic computation (unlike hyperledger)
% Describe why immutable is efficient
% Ergoscript provides powerful features to write complex smart contracts parelling and sometimes surpassing those in Ethereum

% See if we can refer   https://blog.blockstack.org/introducing-clarity-the-language-for-predictable-smart-contracts/

%This is called the {\em UTXO-based} model, because the containers are called {\em unspent transaction outputs} (UTXOs). As the name suggests, UTXOs are the outputs of a transaction that are not yet spent, and can encode arbitrary conditions as long as they are expressible in the underlying script.\footnote{Bitcoin actually discourages such usage of its script, because apart from the few ``standard'' transaction types, a bitcoin core node does not propagate non-standard transactions, even though it accepts a block containing such transactions. Ergo, on the other hand is entirely about non-standard transactions.} A UTXO is uniquely identified by the transaction id and the output index. Funds stored in a UTXO must be spent in entirety. 

%These show that \langname can cover 99\% of the use-cases in Ethereum. 
...
%In an ideal game, both parties reveal their secrets simultaneously, so no party can choose adaptively. In the real-world, however, having the parties reveal their secrets simultaneously is not feasible. Hence, 
%An attack that is often overlooked here is {\em adaptive selection}, where
...
%In particular, the game proceeds in three steps:
...
%We could think of a fourth stage, where the winner claims the prize but this is usually combined with the third stage. 
...
%// below encodes the spending condition of the locked coin
...
%She uses $c$ along with her public key \texttt{alice} to generate \texttt{fullGameScriptHash}, the hash of the following script:
%
%Alice creates a commitment $c=H(a||s)$ to her secret $a$. She uses $c$ along with her public key \texttt{alice} to generate \texttt{fullGameScriptHash}, the hash of the following script:
...
%One day you discover several unauthorized transactions from the hot-wallet, indicating a breach. You wish there was a way to reverse the transactions and cancel the withdraws but alas this is not the case. 
%It is usually impossible to recover the stolen funds once the transaction is included in the blockchain, even if the breach was discovered within minutes. 
%The irreversibility of fund transfers in cryptocurrencies, usually considered a feature, has now become a flaw.
%i, which Such wallets are used for facilitating automated withdrawals and all funds withdrawn by customers originate from this address. Being a hot-wallet, its private key 
%if the server gets compromised. 
% and its funds, which will lead to funds being stolen. %
%What we ideally want is that in the event of such a compromise, 
...
%In \langname, this can done by storing the funds in a special type of address denoted as {\em reversible}. 
...
%Note that this requires $C_a$ to also create and deploy $C_b$. 
...
% ({\em before} the compromise occurred). 
%To use a different trusted party, a new address has to be generated. 
...
%This is a two step protocol, where the final state is represented by the following script called \texttt{withdrawScript}. This will be the guarding script of the hot-wallet's withdraw transaction paying to \texttt{bob}.
...
%However, such addresses should not be used for  %They are not designed for daily spending (such as paying for a coffee). 
...
%Here we show how to implement ICOs in Ergo. 
...
%\with the crowdfunding (Section~\ref{crowfunding}) and ICO (Section~\ref{ico}) contracts to form a fully decentralized autonomous organization.
