...
package impl {
// Abs -----------------------------------
trait CalcsDefs extends scalan.Scalan with Calcs {
  self: Library =>
import Coll._
import Costed._
...
object Calc extends EntityObject("Calc") {
  ...
  // familyElem
  class CalcElem[Val, To <: Calc[Val]](implicit _eVal: Elem[Val])
  ...
  implicit final def calcElement[Val](implicit eVal: Elem[Val]): Elem[Calc[Val]] =
    cachedElemByClass(eVal)(classOf[CalcElem[Val, Calc[Val]]])
  ...
  registerEntityObject("Calc", Calc)

object CalcPrim extends EntityObject("CalcPrim") {
  ...
  // familyElem
  class CalcPrimElem[Val, To <: CalcPrim[Val]](implicit _eVal: Elem[Val])
    extends CalcElem[Val, To] {
    override def eVal = _eVal

    override lazy val parent: Option[Elem[_]] = Some(calcElement(element[Val]))
    override def buildTypeArgs = super.buildTypeArgs ++ TypeArgs("Val" -> (eVal -> scalan.util.Invariant))
  }

  implicit final def calcPrimElement[Val](implicit eVal: Elem[Val]): Elem[CalcPrim[Val]] =
    cachedElemByClass(eVal)(classOf[CalcPrimElem[Val, CalcPrim[Val]]])
  ...
  registerEntityObject("CalcPrim", CalcPrim)

object CalcPair extends EntityObject("CalcPair") {
...
implicit lazy val eR = source.elem.typeArgs("R")._1.asInstanceOf[Elem[R]]
  ...
  registerEntityObject("CalcPair", CalcPair)

object CalcFunc extends EntityObject("CalcFunc") {
...
implicit lazy val eRes = source.elem.typeArgs("Res")._1.asInstanceOf[Elem[Res]]
  ...
  registerEntityObject("CalcFunc", CalcFunc)

object CalcColl extends EntityObject("CalcColl") {
  ...
  registerEntityObject("CalcColl", CalcColl)

object CalcOption extends EntityObject("CalcOption") {
  ...
  registerEntityObject("CalcOption", CalcOption)

object CalcBuilder extends EntityObject("CalcBuilder") {
...
implicit val eRes = func.elem.eRange.typeArgs("Val")._1.asInstanceOf[Elem[Res]]
  ...
  registerEntityObject("CalcBuilder", CalcBuilder)
  ...
  registerModule(CalcsModule)
}

object CalcsModule extends scalan.ModuleInfo("special.collection", "Calcs")
}

trait CalcsModule extends special.collection.impl.CalcsDefs {self: Library =>}
